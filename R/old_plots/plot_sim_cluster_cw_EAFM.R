...
simPars_HCR2 <- read.csv("data/guidelines/SimPars2.4.csv") #simpars for ER tracking umsy, assessed EG, low ER if red
simPars_HCR3 <- read.csv("data/guidelines/SimPars2.5.csv") #simpars for ER tracking umsy, assessed EG low ER if amber or red
...
names(srdat_all)
unique(srdat_all$nameMP)

#recode for pretty label plot
...
srdat_all$plotMP<-dplyr::recode_factor(factor(srdat_all$nameMP),
                                      "10yr_autocorr_HCR1"="Stationary (10y) HCR1",
                                      "5yr_autocorr_HCR1"="Stationary (5y) HCR1",
                                      "10yr_rwa_HCR1"="Time-varying (10y) HCR1",      
                                      "5yr_rwa_HCR1"="Time-varying (5y) HCR1",        
                                      "10yr_both_HCR1"="Mixed (10y) HCR1",
                                      "5yr_both_HCR1"="Mixed (5y) HCR1",
                                      "10yr_autocorr_HCR2"="Stationary (10y) HCR2", 
                                      "5yr_autocorr_HCR2"="Stationary (5y) HCR2", 
                                      "10yr_rwa_HCR2"="Time-varying (10y) HCR2",     
                                      "5yr_rwa_HCR2"="Time-varying (5y) HCR2",
                                      "10yr_both_HCR2"="Mixed (10y) HCR2",
                                      "5yr_both_HCR2"="Mixed (5y) HCR2", 
                                      "10yr_autocorr_HCR3"="Stationary (10y) HCR3",
                                      "5yr_autocorr_HCR3"="Stationary (5y) HCR3",
                                      "10yr_rwa_HCR3"="Time-varying (10y) HCR3",
                                      "5yr_rwa_HCR3"="Time-varying (5y) HCR3",
                                      "10yr_both_HCR3"="Mixed (10y) HCR3",
                                      "5yr_both_HCR3"="Mixed (5y) HCR3") 
                                     
hcrdat_all$plotMP<-dplyr::recode_factor(factor(hcrdat_all$nameMP),
                                      "10yr_autocorr_HCR1"="Stationary (10y) HCR1",
                                      "5yr_autocorr_HCR1"="Stationary (5y) HCR1",
                                      "10yr_rwa_HCR1"="Time-varying (10y) HCR1",      
                                      "5yr_rwa_HCR1"="Time-varying (5y) HCR1",        
...
srdat_all_5yr<-srdat_all[grep(pattern="5y",srdat_all$nameMP),]
hcrdat_all_5yr<-hcrdat_all[grep(pattern="5y",hcrdat_all$nameMP),]
...
st_agg.s<-ggplot(hcrdat_all_5yr[hcrdat_all_5yr$year<111,])+
...
unique(hcrdat_all_5yr$plotMP)
hcrdat.s<-hcrdat_all_5yr
#hcrdat.s=subset(hcrdat_all_5yr,plotOM %in%  c('Static - 0.4AR1','-75%Prod'))
hcrdat.s$plotOM=droplevels(hcrdat.s$plotOM)

hcrdat.s=subset(hcrdat.s,plotMP %in% c("Stationary (5y) HCR1" ,"Time-varying (5y) HCR1","Mixed (5y) HCR1",
                                       "Stationary (5y) HCR2" ,"Time-varying (5y) HCR2","Mixed (5y) HCR2",
                                       "Stationary (5y) HCR3" ,"Time-varying (5y) HCR3","Mixed (5y) HCR3"))
hcrdat.s$plotMP=droplevels(hcrdat.s$plotMP)
hcrdat.s$plotMP=dplyr::recode_factor(factor(hcrdat.s$plotMP),
   "Stationary (5y) HCR1"="Stationary HCR1" ,
  "Time-varying (5y) HCR1"="Time-varying HCR1",
  "Mixed (5y) HCR1"="Mixed HCR1",
  "Stationary (5y) HCR2" ="Stationary HCR2",
  "Time-varying (5y) HCR2"="Time-varying HCR2",
  "Mixed (5y) HCR2"="Mixed HCR2",
  "Stationary (5y) HCR3" ="Stationary HCR3",
  "Time-varying (5y) HCR3"="Time-varying HCR3",
  "Mixed (5y) HCR3"="Mixed HCR3")
  
dplyr::recode_factor(factor(hcrdat.s$rp_type),
  "autocorr"="Stationary" ,
  "rwa"="Time-varying",
  "both"="Mixed")

## assessed umsy & abundace benchmarks####
st_agg.s<-ggplot(hcrdat.s[hcrdat.s$year<111,])+
  geom_bar(aes(x=year-50, fill=wsp.status),position = "fill")+
  scale_fill_manual(values = 
                      statusCols)+
  facet_grid(plotOM~plotMP)+
...
st_agg_all.s<-ggplot(hcrdat.s[hcrdat.s$year<111,])+
...
hcrdat.2scn<-subset(hcrdat.s,plotOM %in%  c('Static - 0.4AR1','-75%Prod'))
unique


st_agg.2scn<-ggplot(hcrdat.2scn[hcrdat.2scn$year<111,])+
  geom_bar(aes(x=year-50, fill=wsp.status),position = "fill")+
  scale_fill_manual(values = 
                      statusCols)+
  facet_grid(plotOM~plotMP)+
...
st_agg_all.2scn<-ggplot(hcrdat.2scn[hcrdat.2scn$year<111,])+
  geom_bar(aes(x=plotMP, fill=wsp.status),position = "fill")+
  scale_fill_manual(values = 
                      statusCols,name='Status')+
  facet_grid(plotOM~.)+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
st_agg_all.2scn
stclass.2scn<-plot_grid(
  st_agg.2scn, st_agg_all.2scn,
  align = "h", axis = "bt",
  rel_widths = c(.75,.25)
)

ggsave("figs_EAFM/status_2scn_allHCR.png",plot=stclass.2scn,width=18,height=5)
...
bmdat_c= hcrdat.s[hcrdat.s$year>59&hcrdat.s$year<111,] %>% 
group_by(plotOM,plotMP,iteration,HCR,rp_type)  %>% 
...
allpm<-full_join(catch_c,bmdat_c)
names(allpm)
#"m.catch",  "aav.catch",
allpm_long<-allpm |> 
     select("scale.ann.catch", "p.red","p.bm", "plotOM", "plotMP", "iteration", "HCR","rp_type" )|>
     tidyr::pivot_longer(!c("plotOM", "plotMP","iteration", "HCR","rp_type" ))

#Figures: trade-off plots in catch vs. conservation risk (% below upper BM)#####
names(allpm_long)

ggplot(allpm_long)+
geom_boxplot(aes(x=HCR,y=value, fill=as.factor(rp_type), group=plotMP))+
theme_bw(15)+
...
names(hcrdat.s)
unique(hcrdat.s$HCR)
unique(hcrdat.s$rp_type)

all_comp_cat=expand.grid(mp=levels(hcrdat.s$plotMP),scn=levels(hcrdat.s$plotOM),
  value=NA,
  l95=NA,u95=NA)

all_comp_cat$rp_type<-gsub(" HCR\\d+", "", all_comp_cat$mp)
all_comp_cat$HCR<-gsub(".*(\\d)$", "\\1", all_comp_cat$mp)

all_comp_p.red=expand.grid(mp=levels(hcrdat.s$plotMP),scn=levels(hcrdat.s$plotOM),value=NA,
  l95=NA,u95=NA)
all_comp_p.red$rp_type<-gsub(" HCR\\d+", "", all_comp_p.red$mp)
all_comp_p.red$HCR<-gsub(".*(\\d)$", "\\1", all_comp_p.red$mp)



all_comp_p.ub=expand.grid(mp=levels(hcrdat.s$plotMP),scn=levels(hcrdat.s$plotOM),value=NA,
  l95=NA,u95=NA)
all_comp_p.ub$rp_type<-gsub(" HCR\\d+", "", all_comp_p.ub$mp)
all_comp_p.ub$HCR<-gsub(".*(\\d)$", "\\1", all_comp_p.ub$mp)


for(i in 1:length(levels(hcrdat.s$plotOM))){

  #dattt=subset(allpm,plotOM==levels(allpm$plotOM)[i])

  ests=lm(scale.ann.catch~plotMP-1,data=subset(allpm,plotOM==levels(allpm$plotOM)[i]))
  confs=confint(ests)
  
  all_comp_cat[match(all_comp_cat$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,3]<-c(ests$coefficients)#,
                    

  #these are wrong, only tight because of large number or iterations
  all_comp_cat[match(all_comp_cat$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,4]=c(confs[,1])
                                                                  
  all_comp_cat[match(all_comp_cat$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,5]=c(confs[,2])
  ...
  all_comp_p.red[match(all_comp_p.red$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,3]<-c(ests2$coefficients)

  
  all_comp_p.red[match(all_comp_p.red$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,4]=c(confs2[,1])
  all_comp_p.red[match(all_comp_p.red$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,5]=c(confs2[,2])


  ests3=lm(p.bm~plotMP-1,data=subset(allpm,plotOM==levels(allpm$plotOM)[i]))
  confs3=confint(ests3)
  all_comp_p.ub[match(all_comp_p.ub$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,3]<-c(ests3$coefficients)

  
  all_comp_p.ub[match(all_comp_p.ub$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,4]=c(confs3[,1])
  all_comp_p.ub[match(all_comp_p.ub$scn,levels(hcrdat.s$plotOM)[i],nomatch =0)==1,5]=c(confs3[,2])
...
all_comp_p.red$value_scaled<-NA
all_comp_p.ub$value_scaled<-NA
all_comp_cat$value_scaled<-NA



for(sc in unique(all_comp_cat$scn)){

  tmpcat<-all_comp_cat[all_comp_cat$scn==sc,"value"] 
  tmpred<-all_comp_p.red[all_comp_p.red$scn==sc,"value"] 

  all_comp_cat[all_comp_cat$scn==sc,"value_scaled"] <-min( tmpred)+((tmpcat - min(tmpcat))*(max( tmpred) - min( tmpred))) / (max(tmpcat) - min(tmpcat))

  tmpred<-all_comp_p.red[all_comp_p.red$scn==sc,"value"] 
  all_comp_p.red[all_comp_p.red$scn==sc,"value_scaled"] <-(tmpred)# - min(tmpred)) / (max(tmpred) - min(tmpred))

  tmpub<-all_comp_p.ub[all_comp_p.ub$scn==sc,"value"] 
  all_comp_p.ub[all_comp_p.ub$scn==sc,"value_scaled"] <-(tmpub)#- min(tmpub)) / (max(tmpub) - min(tmpub))
}


all_comp_d<-rbind(all_comp_p.red,all_comp_cat)

droplevels(all_comp_d$scn)




lb_cat_tradeoff<-ggplot( all_comp_d, aes(y=value_scaled, x=variable, colour=rp_type,group=mp)) + 
...
ggsave("figs_EAFM/tradeoff_lb_cat_all.png",plot=lb_cat_tradeoff,width=10,height=33)


names(all_comp_d)
all_comp_d_2scn<-subset(all_comp_d,scn %in%  c('Static - 0.4AR1','-75%Prod'))

lb_cat_tradeoff_2scn<-ggplot(all_comp_d_2scn, aes(y=value_scaled, x=variable, colour=rp_type,group=mp)) + 
    #geom_errorbar(aes(ymin=l95, ymax=u95, colour=rp_type), width=.1) +
    #geom_line(aes(y=as.numeric(value), x=variable, colour=mp)) +
    stat_summary(fun=max, geom="line",linewidth=2)+
    theme_bw(15) +
...
lb_cat_tradeoff_2scn



ggsave("figs_EAFM/tradeoff_lb_cat_2scn.png",plot=lb_cat_tradeoff_2scn,width=12,height=5)
