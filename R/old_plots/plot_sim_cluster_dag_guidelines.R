...
hcrdat_um<-hcrdat_um[hcrdat_um$year>50,] #remove burn-in period
...
#Figures: true status####
##tracks umsy - no abundance benchmark####
...
##set target - varying abundance benchmark####
...
##tracks umsy & fixed abundance benchmark####
...
stclass
ggsave("outputs/figs/true_status_ERtar0.9UMSY_0.3fixedBM.png",plot=stclass,width=14,height=8.5)

#Figures: Guidance doc outputs with a limited set of scenarios####

##true status subset - assessed abundance benchmark (set 2)####
hcrdat2x=subset(hcrdat2,plotOM %in% c('Static - 0.4AR1','Static - 0.8AR1','-50%Cap','-75%Cap','+50%Prod','-50%Prod','-75%Prod'))
...
stclass
ggsave("./outputs/figs/GDsub_true_status_ERtar0.75_0.1BM.png",plot=stclass,width=14,height=8.5)

##true status subset - umsy & fixed abundance benchmark ####
hcrdat4x=subset(hcrdat4,plotOM %in% c('Static - 0.4AR1','Static - 0.8AR1','-50%Cap','-75%Cap','+50%Prod','-50%Prod','-75%Prod'))
...
stclass
ggsave("./outputs/figs/GDsub_true_status_ERtar1.05Umsy_0.3fixEG.png",plot=stclass,width=14,height=8.5)


##estimated status - abundance benchmark####
st_agg<-ggplot(hcrdat2x[hcrdat2x$year<111,])+
...
theme_bw(14)
upperBMtrue_hcr
ggsave("outputs/figs/upperBM_true_hcr_ERtar0.75_0.1BM.png",plot=upperBMtrue_hcr,width=14,height=8.5)
...
theme_bw(14)
lowerBMtrue_hcr
ggsave("outputs/figs/lowerBM_true_hcr_ERtar0.75_0.1BM.png",plot=lowerBMtrue_hcr,width=14,height=8.5)
...
srdat_b$plotOM<-dplyr::recode_factor(factor(srdat_b$nameOM),
                                     "stationarylAR1"  = "Static - 0.4AR1",
                                     "stationaryhAR1" = "Static - 0.8AR1",
                                     "decLinearcap0.50"="-50%Cap", 
                                     "decLinearcap0.25"="-75%Cap",
...
catch_plot_scd
ggsave("outputs/figs/catch_violins_ERtar1.05UMSY_0.5fixedBM.png",plot=catch_plot_scd,width=8,height=12)
...
catch_d_s=subset(catch_d,plotOM %in% levels(hcrdat4.s$plotOM))
...
bmdat_d_s=subset(bmdat_d,plotOM %in% levels(hcrdat4.s$plotOM))
...
plot(c(-10,250)~c(1,2),bty='l',type='n',xaxt='n',ylab='% Difference from stationary',xlab='',ylim=c(-10,250))
abline(h=0)
abline(h=50,lty=5,col=adjustcolor('black',alpha.f = 0.5))
abline(h=100,lty=5,col=adjustcolor('black',alpha.f = 0.5))
...
png(filename='outputs/figs/catch_conservation_tradeoff_spawner_ERtar0.75_0.1BM.png',width=8,height=6,units='in',res=600)
...
for(i in 1:length(levels(hcrdat4.s$plotOM))){
  ests=lm(m.catch~plotMP-1,data=subset(catch_d_s,plotOM==levels(catch_d_s$plotOM)[i]))
  confs=confint(ests)
...
for(i in 1:length(levels(hcrdat4.s$plotOM))){
  ests=lm(p.bm~plotMP-1,data=subset(bmdat_d_s,plotOM==levels(bmdat_d_s$plotOM)[i]))
  confs=confint(ests)
  ...
  ests2=lm(1-p.red~plotMP-1,data=subset(bmdat_d,plotOM==levels(bmdat_d$plotOM)[i]))
  confs2=confint(ests2)
...
#  data.frame(scn=levels(hcrdat4.s$plotOM),status.10y.s=NA,status.10y.rwa=NA,status.10y.rwa.l95=NA,status.10y.rwa.u95=NA,status.5y.s=NA,status.5y.s.l95=NA,status.5y.s.u95=NA,status.5y.rwa=NA,status.5y.rwa.l95=NA,status.5y.rwa.u95=NA)
for(i in 1:length(levels(hcrdat4.s$plotOM))){
  ests=lm(m.spawners~plotMP-1,data=subset(spawners_d_s,plotOM==levels(spawners_d_s$plotOM)[i]))
  confs=confint(ests)
...
png(filename='outputs/figs/catch_conservation_tradeoff_spawner_ERtar0.9UMSY_0.3fixedBM.png',width=8,height=6,units='in',res=600)
plot(c(-30,20)~c(1,2),bty='l',type='n',xaxt='n',ylab='% Difference from stationary',xlab='',ylim=c(-30,20))
abline(h=0)
abline(h=10,lty=5,col=adjustcolor('black',alpha.f = 0.5))
abline(h=20,lty=5,col=adjustcolor('black',alpha.f = 0.5))
abline(h=30,lty=5,col=adjustcolor('black',alpha.f = 0.5))
...
catch_plot_scb<-ggplot(catch_b_s,aes(x=plotMP,y=scale.ann.catch,fill=plotMP))+
...
stclass<-plot_grid(
  st_agg, st_agg_all+ theme(legend.position="none"),catch_plot_scb,
  align='hv',axis='bt',ncol=3,
  rel_widths = c(.5,.25,.25)
)

stclass
ggsave("./outputs/figs/composite_plot_status_catch_ERtar0.75_0.1BM.png",plot=stclass,width=14,height=8.5)
...
catch_plot_scd<-ggplot(catch_d_s,aes(x=plotMP,y=scale.ann.catch,fill=plotMP))+
  scale_fill_manual(values=c('dodgerblue4','goldenrod2')) +
  geom_violin()+
  #  geom_boxplot(width=0.1, color="white", alpha=0.5) +
  stat_summary(fun= median,
...
stclass<-plot_grid(
  st_agg, st_agg_all+ theme(legend.position="none"),catch_plot_scd,
  align = "hv", axis = "bt",ncol=3,
  rel_widths = c(.5,.25,.25)
)
...
#combined - fxixed B/assessed F ref pts
